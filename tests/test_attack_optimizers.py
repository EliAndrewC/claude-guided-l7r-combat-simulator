...
class TestAttackOptimizer(unittest.TestCase):
    ...
    def test_cannot_reach_threshold(self):
        ...
        optimizer = AttackOptimizer(self.taro, self.jiro, "double attack", self.initiative_action, self.context)
        ...
        # P(50|9k6) = 0.38
        # Recommendation for threshold 0.5 should be None
        optimizer = AttackOptimizer(self.jiro, self.bayushi, "double attack", self.initiative_action, self.context)
        self.assertEqual(None, optimizer.optimize(0.5))
    ...
    def test_spend_no_resources(self):
        ...
        optimizer = AttackOptimizer(self.bayushi, self.taro, "double attack", self.initiative_action, self.context)
        ...
        optimizer = AttackOptimizer(self.bayushi, self.taro, "attack", self.initiative_action, self.context)
    ...
    def test_spend_some_resources(self):
        ...
        optimizer = AttackOptimizer(self.taro, self.jiro, "attack", self.initiative_action, self.context)
        ...
        optimizer = AttackOptimizer(self.taro, self.bayushi, "attack", self.initiative_action, self.context)
        attack = optimizer.optimize(0.6)
        self.assertEqual(1, attack.vp())
...
class TestDamageOptimizer(unittest.TestCase):
    ...
    def test_cannot_reach_threshold(self):
        ...
        # P(45|8k5) = 0.31
        # Recommendation for threshold 0.5 should be None
        optimizer = DamageOptimizer(self.taro, self.jiro, "double attack", self.initiative_action, self.context)
        self.assertEqual(None, optimizer.optimize(0.5))
        #
        # Jiro double attacking Bayushi has TN 50
        # Jiro's base attack roll is 7k4, spending 2 VP would reach 9k6
        # P(50|9k6) = 0.38
        # Recommendation for threshold 0.5 should be None
        optimizer = DamageOptimizer(self.jiro, self.bayushi, "double attack", self.initiative_action, self.context)
        self.assertEqual(None, optimizer.optimize(0.5))
    ...
    def test_spend_no_resources(self):
        ...
        optimizer = DamageOptimizer(self.taro, self.jiro, "attack", self.initiative_action, self.context)
        ...
        optimizer = DamageOptimizer(self.bayushi, exploding_mook, "attack", self.initiative_action, self.context)
        attack = optimizer.optimize(0.9)
        self.assertEqual(0, attack.vp())
    ...
    def test_spend_all_resources(self):
        ...
        optimizer = DamageOptimizer(self.jiro, self.bayushi, "attack", self.initiative_action, self.context)
        attack = optimizer.optimize(0.5)
        self.assertEqual(2, attack.vp())

...
class TestModifierEvents(unittest.TestCase):
    ...
    def test_listen_to_events(self):
        ...
        irrelevant_attack = actions.AttackAction(self.target, self.subject, "attack", self.initiative_action, self.context)
        irrelevant_attack_event = events.AttackSucceededEvent(irrelevant_attack)
        list(modifier.handle(self.subject, irrelevant_attack_event, self.context))
        # should have no effect
        self.assertEqual(0, self.subject.sw())
        self.assertEqual(0, self.target.sw())
        # play a relevant event on the listener
        relevant_attack = actions.AttackAction(self.subject, self.target, "attack", self.initiative_action, self.context)
        relevant_attack_event = events.AttackSucceededEvent(relevant_attack)
        list(modifier.handle(self.subject, relevant_attack_event, self.context))

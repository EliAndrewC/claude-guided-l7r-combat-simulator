...
class TestContestedIaijutsuAttackAction(unittest.TestCase):
    ...
    def setUp(self):
        # set characters
        kakita = CharacterBuilder(xp=9001).with_name("Kakita").with_school(KakitaBushiSchool()).buy_skill("double attack", 5).buy_skill("iaijutsu", 5).buy_skill("lunge", 5).build()
        matsu = CharacterBuilder(xp=9001).with_name("Matsu").generic().buy_skill("iaijutsu", 5).build()
        # set context
    ...
    def test_calculate_extra_damage_dice_outroll(self):
        ...
        action = ContestedIaijutsuAttackAction(self.kakita, self.matsu, self.kakita, "iaijutsu", "iaijutsu", self.initiative_action, self.context)
    ...
    def test_penalize_no_iaijutsu(self):
        ...
        action = ContestedIaijutsuAttackAction(self.kakita, self.matsu, self.kakita, "attack", "iaijutsu", self.initiative_action, self.context)
...
class TestKakitaAttackAction(unittest.TestCase):
    ...
    def setUp(self):
        # set characters
        kakita = CharacterBuilder(xp=9001).with_name("Kakita").with_school(KakitaBushiSchool()).buy_ring("fire", 4).buy_skill("attack", 3).buy_skill("double attack", 3).buy_skill("iaijutsu", 3).buy_skill("lunge", 3).build()
    ...
    def test_equal_tempo(self):
        ...
        attack = KakitaAttackAction(self.kakita, self.target, "iaijutsu", initiative_action, context)
...
class TestKakitaNewPhaseListener(unittest.TestCase):
    ...
    def setUp(self):
        # characters
        kakita = CharacterBuilder(xp=9001).with_name("Kakita").with_school(KakitaBushiSchool()).buy_ring("fire", 5).buy_skill("double attack", 5).buy_skill("iaijutsu", 5).buy_skill("lunge", 5).build()
        target = Character("target")
        # context
...
class TestKakitaRollParameterProvider(unittest.TestCase):
    ...
    def setUp(self):
        kakita = CharacterBuilder(xp=9001).with_name("Kakita").with_school(KakitaBushiSchool()).buy_skill("double attack", 4).buy_skill("iaijutsu", 4).buy_skill("lunge", 4).buy_ring("fire", 5).build()
        target = Character("target")
        self.kakita = kakita
        self.target = target
    ...
    def test_iaijutsu_damage_bonus(self):
        ...
        (rolled, kept, bonus) = provider.get_damage_roll_params(self.kakita, self.target, "iaijutsu", 0)
    ...
    def test_other_skill_no_damage_bonus(self):
        provider = KakitaRollParameterProvider()
        (rolled, kept, bonus) = provider.get_damage_roll_params(self.kakita, self.target, "attack", 0)
        self.assertEqual(9, rolled)
        self.assertEqual(2, kept)
...
class TestTakeContestedIaijutsuAttackAction(unittest.TestCase):
    def setUp(self):
        kakita = CharacterBuilder(xp=9001).with_name("Kakita").with_school(KakitaBushiSchool()).buy_skill("double attack", 5).buy_skill("iaijutsu", 5).buy_skill("lunge", 5).buy_ring("fire", 5).build()
        target = Character("target")
    ...
    def test_play(self):
        ...
        #  1. NewPhaseEvent
        self.assertTrue(isinstance(history[0], NewPhaseEvent))
        #  2. TakeContestedIaijutsuAttackAction
        take_attack_event = history[1]
        self.assertTrue(isinstance(take_attack_event, TakeContestedIaijutsuAttackAction))
        #  3. ContestedIaijutsuDeclaredEvent(challenger, defender)
        challenger_declared_event = history[2]
        self.assertTrue(isinstance(challenger_declared_event, ContestedIaijutsuAttackDeclaredEvent))
        self.assertEqual(self.kakita, challenger_declared_event.action.subject())
        self.assertEqual(self.target, challenger_declared_event.action.target())
        self.assertEqual(self.kakita, challenger_declared_event.action.challenger())
        self.assertEqual("iaijutsu", challenger_declared_event.action.skill())
        self.assertEqual("attack", challenger_declared_event.action.contested_skill())
        #  4. ContestedIaijutsuDeclaredEvent(defender, challenger)
        defender_declared_event = history[3]
        self.assertTrue(isinstance(defender_declared_event, ContestedIaijutsuAttackDeclaredEvent))
        self.assertEqual(self.target, defender_declared_event.action.subject())
        self.assertEqual(self.kakita, defender_declared_event.action.target())
        self.assertEqual(self.kakita, defender_declared_event.action.challenger())
        self.assertEqual("attack", defender_declared_event.action.skill())
        self.assertEqual("iaijutsu", defender_declared_event.action.contested_skill())
        #  5. ContestedIaijutsuRolledEvent(challenger, defender)
        challenger_rolled_event = history[4]
        self.assertTrue(isinstance(challenger_rolled_event, ContestedIaijutsuAttackRolledEvent))
        self.assertEqual(self.kakita, challenger_rolled_event.action.subject())
        self.assertEqual(self.target, challenger_rolled_event.action.target())
        self.assertEqual(self.kakita, challenger_rolled_event.action.challenger())
        self.assertEqual("iaijutsu", challenger_rolled_event.action.skill())
        self.assertEqual("attack", challenger_rolled_event.action.contested_skill())
        self.assertEqual(challenger_skill_roll + 5, challenger_rolled_event.action.skill_roll())
        self.assertEqual(defender_skill_roll, challenger_rolled_event.action.opponent_skill_roll())
        #  6. ContestedIaijutsuRolledEvent(defender, challenger)
        defender_rolled_event = history[5]
        self.assertTrue(isinstance(defender_rolled_event, ContestedIaijutsuAttackRolledEvent))
        self.assertEqual(self.target, defender_rolled_event.action.subject())
        self.assertEqual(self.kakita, defender_rolled_event.action.target())
        self.assertEqual(self.kakita, defender_rolled_event.action.challenger())
        self.assertEqual("attack", defender_rolled_event.action.skill())
        self.assertEqual("iaijutsu", defender_rolled_event.action.contested_skill())
        self.assertEqual(defender_skill_roll, defender_rolled_event.action.skill_roll())
        self.assertEqual(challenger_skill_roll + 5, defender_rolled_event.action.opponent_skill_roll())
        #  7. LightWoundsDamageEvent
        damage_event = history[6]
        self.assertTrue(isinstance(damage_event, LightWoundsDamageEvent))

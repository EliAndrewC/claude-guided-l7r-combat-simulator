...
if not available_names:
    ...
    with col1:
        ...
        control_names = st.multiselect("Control group characters", available_names, key="control_chars")

    with col2:
        st.subheader("Test Group")
        test_names = st.multiselect("Test group characters", available_names, key="test_chars")

    # Validation
    overlap = set(control_names) & set(test_names)
    if overlap:
        st.error(f"Characters cannot be in both groups: {', '.join(overlap)}")
    elif not control_names:

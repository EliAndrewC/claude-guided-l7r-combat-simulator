...
LOG_LEVELS = {"critical": logging.CRITICAL, "error": logging.ERROR, "fatal": logging.FATAL, "warn": logging.WARN, "info": logging.INFO, "debug": logging.DEBUG}
...
def load_characters(dirpath):
    ...
    for fname in fnames:
        ...
        if not os.path.isfile(fpath):
            logger.warn(f"Skipping invalid input file: {fpath}")
            continue
...
def load_groups(dirpath, characterd):
    ...
    if not os.path.exists(fpath):
        logger.error(f"groups.yaml not found in input directory {dirpath}")
        sys.exit(1)
    with open(fpath) as f:
...
def main():
    ...
    if not os.path.isdir(args.input_dirpath):
        error(f"Invalid input directory path {args.input_dirpath}")
        sys.exit(1)
    ...
    if not os.path.exists(args.output_dirpath):
        ...
        if not os.path.isdir(args.output_dirpath):
            error(f"Output directory already exists but is not a directory: {args.output_dirpath}")
            sys.exit(1)
    ...
    print(f"Test group ({test_group_characters}) won {test_victories}/{ntrials} trials.")
